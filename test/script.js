document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateSubtitle(currentTime) {
    ...
    if (newSubtitleIndex !== currentSubtitleIndex) {
      ...
      if (currentSubtitleIndex !== -1) {
        subtitleContainer.textContent = subtitles[currentSubtitleIndex].text;

        // 更新画中画窗口中的字幕
        if (pipWindow && !pipWindow.closed) {
          pipWindow.postMessage(
            { subtitle: subtitles[currentSubtitleIndex].text },
            "*"
          );
        }
      } else {
        ...
        // 更新画中画窗口中的字幕
        if (pipWindow && !pipWindow.closed) {
          pipWindow.postMessage({ subtitle: "" }, "*");
        }
      }
    }
  }
              ...
              <body>
                ...
                <script>
                  window.addEventListener('message', function(event) {
                    document.getElementById('subtitle-content').textContent = event.data.subtitle;
                  });
                </script>
              </body>
            </html>
  ...
  backgroundFileBtn.addEventListener("click", function () {
    ...
    playerContainer.classList.toggle("transparent-style");
  });
});
